...
#include "./main_icon.h"
#include "./textra_font.h"
#include "./museo_sans_cyrl_500.h"
...
bool ImGui::HotKey(const char* szLabel, unsigned int* pValue)
{
	...
	const ImRect rectFrame(pWindow->DC.CursorPos + ImVec2(startBox, 0.0f), pWindow->DC.CursorPos + ImVec2(startBox + box_width, h));
	...
	pWindow->DrawList->AddText(ImVec2(rectFrame.Max.x - vecBufferSize.x, rectTotal.Min.y + style.FramePadding.y), GetColorU32(ImGuiCol_Text), szBuffer);

	if (vecLabelSize.x > 0.f)
		RenderText(ImVec2(window->DC.CursorPos.x, p.y), szLabel);
...
bool ImGui::HotKey(const char* szLabel, KeyBind_t* pKeyBind, const bool bAllowSwitch)
{
	...
	if (bAllowSwitch)
	{
		...
		{
			SetNextItemWidth(ImGui::GetWindowWidth() * 2.5f);
...
#pragma endregion
	...
	imVerdanaConfig.FontBuilderFlags = ImGuiFreeTypeBuilderFlags_ForceAutoHint;
	imVerdanaConfig.FontDataOwnedByAtlas = false;
	for (int i = 0; i < CS_ARRAYSIZE(FONT::pMenu); i++)
	{
		const float flFontSize = 15.f * CalculateDPI(i);
		FONT::pMenu[i] = io.Fonts->AddFontFromMemoryTTF((void*)museo_sans_cyrl_500, sizeof(museo_sans_cyrl_500), flFontSize, &imVerdanaConfig, io.Fonts->GetGlyphRangesCyrillic());
		io.Fonts->AddFontFromMemoryTTF((void*)fa_solid_900, sizeof(fa_solid_900), flFontSize, &icons_config, icons_ranges);
	}

	FONT::pCheatLogo = io.Fonts->AddFontFromMemoryTTF((void*)textra_font, sizeof(textra_font), 190.f, &imVerdanaConfig, io.Fonts->GetGlyphRangesCyrillic());
	...
	FONT::pVisual = io.Fonts->AddFontFromFileTTF(CS_XOR("C:\\Windows\\Fonts\\Tahoma.ttf"), 17.f, &imTahomaConfig, io.Fonts->GetGlyphRangesCyrillic());


	FONT::ico_button = io.Fonts->AddFontFromMemoryTTF((void*)icon, sizeof(icon), 25.f, NULL, io.Fonts->GetGlyphRangesCyrillic());

	FONT::ico_main_logo = io.Fonts->AddFontFromMemoryTTF((void*)main_icon_font, sizeof(main_icon_font), 50, NULL, io.Fonts->GetGlyphRangesCyrillic());

	/*ImFontConfig icon_font_cfg;
	icon_font_cfg.FontDataOwnedByAtlas = false;
	ImWchar weapon_icons_ranges[] = { 0xE000, 0xE204, 0 };
	FONT::pWeaponIcon = io.Fonts->AddFontFromMemoryTTF((void*)csgo_icons, sizeof(csgo_icons), 16, &icon_font_cfg, icons_ranges);*/


	ImFontConfig name_cfg;
	name_cfg.FontBuilderFlags = ImGuiFreeTypeBuilderFlags_ForceAutoHint;
	name_cfg.FontDataOwnedByAtlas = false;
	FONT::pEspIcons = io.Fonts->AddFontFromMemoryTTF((void*)csgo_icons, sizeof(csgo_icons), 11.f, &name_cfg, io.Fonts->GetGlyphRangesDefault());
