...
static bool Setup(HMODULE hModule)
{
	...
	if (!L::AttachConsole(CS_XOR(L"xenon developer-mode")))
	{
...
#endif
#ifdef CS_LOG_FILE
	if (!L::OpenFile(CS_XOR(L"xenon.log")))
	{
	...
	if (CRT::StringCompare(I::Engine->GetProductVersionString(), CS_PRODUCTSTRINGVERSION) != 0)
		L_PRINT(LOG_WARNING) << L::SetColor(LOG_COLOR_FORE_YELLOW | LOG_COLOR_FORE_INTENSITY) << CS_XOR("version mismatch! local CS2 version: ") << CS_PRODUCTSTRINGVERSION << CS_XOR(", current CS2 version: ") << I::Engine->GetProductVersionString() << CS_XOR(". xenon might not function as normal.");

	L_PRINT(LOG_NONE) << L::SetColor(LOG_COLOR_FORE_CYAN | LOG_COLOR_FORE_INTENSITY) << CS_XOR("xenon initialization completed, version: ") << CS_STRINGIFY(CS_VERSION);
...
DWORD WINAPI PanicThread(LPVOID lpParameter)
{
	...
	L_PRINT(LOG_INFO) << CS_XOR("\"Unload Cheat\"");
