...
namespace MENU
{
	...
	{
		ParticleData_t(const ImVec2& vecPosition, const ImVec2& vecVelocity, const float& radius) :
			vecPosition(vecPosition), vecVelocity(vecVelocity), radius(radius) { }

		// current particle position
		ImVec2 vecPosition = {};
		// current particle velocity
		ImVec2 vecVelocity = {0.f, 0.f};

		ImVec2 vecAccelaretion = {};
		ImVec2 vecGravity = { 0.f, 0.005f };
...
namespace T
{
	...
	void VisualsPlayers();
	void VisualsWeapons();
	void VisualsGrenades();
	void VisualsWorld();
	void VisualsView();
	void MiscellaneousMain();
	void MiscellaneousInventory();
	void Configs();
	void MiscellaneousConfigs();
	void Miscellaneous();
	void SkinsChanger();
